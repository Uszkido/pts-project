        ...
        return (
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
                <div className="w-full max-w-md bg-slate-900 border border-emerald-500/20 rounded-3xl p-8 text-center">
                    ...
                    <p className="text-sm text-slate-500 mb-4">You can now proceed to log in. Your account is active but pending final verification of your business documents by an administrator.</p>
                    ...
                    {isOtpStep ? (
                        <form onSubmit={handleVerifyOtp} className="space-y-6">
                            <div className="bg-emerald-500/10 border border-emerald-500/20 p-4 rounded-xl mb-6">
                                <p className="text-emerald-400 text-xs text-center font-medium">{success}</p>
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-slate-400 mb-2">Verification Code (OTP)</label>
                                <input type="text" value={otp} onChange={e => setOtp(e.target.value)} placeholder="123456" className="w-full bg-slate-950/50 border border-slate-700/50 rounded-xl px-4 py-3.5 text-white text-center text-3xl tracking-[0.4em] font-mono focus:outline-none focus:ring-1 focus:ring-blue-500 transition-colors" required maxLength={6} />
                            </div>
                            <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 rounded-xl transition-all disabled:opacity-50">
                                {loading ? 'Verifying...' : 'Complete Verification'}
                            </button>
                            <p className="text-center text-xs text-slate-500 mt-4">If you haven't received your code, please contact your regional administrator for manual verification.</p>
                        </form>
                    ) : (
                        <>
                            {/* Step Progress and forms ... existing code ... */}
