...
export default function ConsumerDashboard() {
    ...
    const acceptTransfer = async (transferId: string) => {
        try {
            const res = await fetch(`http://localhost:5000/api/v1/transfers/accept/${transferId}`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${localStorage.getItem('pts_token')}` }
            });
            if (!res.ok) {
    ...
    const emergencyFreeze = async (deviceId: string) => {
        ...
        try {
            const res = await fetch('http://localhost:5000/api/v1/incidents/emergency-freeze', {
                method: 'POST',
                headers: {
        ...
        <div className="min-h-screen bg-slate-950 font-sans text-slate-200">
            <nav className="border-b border-emerald-900/40 bg-slate-900/80 backdrop-blur-md px-6 py-4 flex justify-between items-center sticky top-0 z-50">
                ...
                <div className="flex items-center gap-4">
                    <button onClick={() => { localStorage.removeItem('pts_token'); window.location.href = '/consumer/login'; }} className="text-sm font-medium text-slate-400 hover:text-white bg-slate-800/50 hover:bg-slate-800 px-4 py-2 rounded-lg transition-colors">Sign Out</button>
                </div>
            </nav>
            ...
            <main className="max-w-5xl mx-auto p-4 sm:p-6 mt-6 sm:mt-10">
                ...
                {pendingTransfers.length > 0 && (
                    <div className="mb-10 bg-amber-500/10 border border-amber-500/20 rounded-2xl p-6 shadow-xl">
                        ...
                        <div className="space-y-4">
                            {pendingTransfers.map((tx: any) => (
                                <div key={tx.id} className="flex flex-col sm:flex-row sm:items-center justify-between bg-slate-900/80 p-5 rounded-xl border border-slate-700/50">
                                    <div>
                                    ...
                                    <button onClick={() => acceptTransfer(tx.id)} className="mt-4 sm:mt-0 bg-amber-600 hover:bg-amber-500 text-white font-bold py-2.5 px-6 rounded-xl transition-colors shadow-lg shadow-amber-500/20">Accept Ownership</button>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {loading ? (
                        <div className="col-span-full text-center py-10 text-slate-500">Decrypting vault contents...</div>
                    ) : devices.length === 0 ? (
                        <div className="col-span-full border border-dashed border-slate-700 bg-slate-900/30 rounded-2xl p-10 text-center">
                            <p className="text-slate-400 font-medium">No assets registered to your digital identity.</p>
                        </div>
                    ) : (
                        devices.map((device: any) => {
                            const activeCert = device.certificates && device.certificates.length > 0 ? device.certificates[0] : null;
                            const isRecoverableStatus = ['INVESTIGATING', 'LOST', 'STOLEN'].includes(device.status);
                                        ...
                                        {activeCert && activeCert.isActive ? (
                                            <div className="flex gap-3 items-center">
                                                ...
                                                <div className="overflow-hidden">
                                                    <div className="text-xs text-white font-semibold truncate leading-tight">ACTIVE</div>
                                    ...
                                    <div className="mt-6 flex flex-col gap-3 relative z-10">
                                        ...
                                        {device.status === 'CLEAN' ? (
                                            ...
                                            <div className="p-3 rounded-xl bg-slate-800/50 border border-slate-700/50 text-center text-sm text-slate-400 cursor-not-allowed">
                                                Transfers Locked due to {device.status} status.
                                            </div>
                                        )}
                                    </div>
                                </div>
                                            ...
                                            <div key={entry.id} className="relative">
                                                ...
                                                <div className="bg-slate-800/40 border border-slate-800 p-4 rounded-xl">
                                                    <div className="text-xs font-bold text-slate-300 uppercase tracking-wide mb-1 flex items-center gap-2">
                                                        ...
                                                        <span className="text-slate-500 lowercase font-normal">by {entry.actor?.companyName || entry.actor?.email}</span>
                                                    </div>
