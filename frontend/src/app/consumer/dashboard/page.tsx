...
export default function ConsumerDashboard() {
    ...
    // UI tabs
    const [activeTab, setActiveTab] = useState<'active' | 'history' | 'register' | 'intelligence' | 'escrow'>('active');
    ...
    const [isThinkingLegal, setIsThinkingLegal] = useState(false);
    ...
    const runScamShield = async () => {
        if (!scamText) return;
        setIsAnalyzingScam(true);
        setScamResult(null);
        try {

            const res = await fetch(`${apiUrl}/ai/scam-shield`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('pts_token')}`
                },
                body: JSON.stringify({ messageText: scamText })
            });
            const data = await res.json();
            setScamResult(data);
        } catch (err: any) {
            alert("Shield Offline: " + err.message);
        } finally {
    ...
    const getLegalHelp = async () => {
        ...
        setIsThinkingLegal(true);
        setLegalAdvice('');
        try {

            const res = await fetch(`${apiUrl}/ai/legal-advisor`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('pts_token')}`
            ...
            alert("Legal AI Busy: " + err.message);
        } finally {
            setIsThinkingLegal(false);
        }
    };
    ...
    const downloadCertificate = async (device: any) => {
        ...
        const activeCert = device.certificates?.find((c: any) => c.isActive);
    ...
    return (
        <div className="min-h-screen bg-slate-950 font-sans text-slate-200">
            ...
            <main className="max-w-5xl mx-auto p-4 sm:p-6 mt-6 sm:mt-10">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 border-b border-slate-800 pb-4">
                    ...
                    <div className="flex flex-wrap bg-slate-900 border border-slate-800 rounded-lg p-1 gap-1">
                        ...
                        <button onClick={() => setActiveTab('intelligence')} className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'intelligence' ? 'bg-purple-600/20 text-purple-400 shadow-md' : 'text-slate-400 hover:text-white'}`}>AI Intelligence</button>
                        <button onClick={() => setActiveTab('escrow')} className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'escrow' ? 'bg-indigo-600/20 text-indigo-400 shadow-md' : 'text-slate-400 hover:text-white'}`}>Safe-Trade Hub</button>
                        <button onClick={() => setActiveTab('history')} className={`px-4 py-2 rounded-md text-sm font-bold transition-all ${activeTab === 'history' ? 'bg-blue-600/20 text-blue-400 shadow-md' : 'text-slate-400 hover:text-white'}`}>History</button>
                    ...
                    <>
                        {activeTab === 'active' &&
                            <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
                                {/* Guardian Mesh Banner */}
                                <div className={`p-4 rounded-2xl border transition-all ${meshEnabled ? 'bg-emerald-500/10 border-emerald-500/30 shadow-lg shadow-emerald-500/10' : 'bg-slate-900 border-slate-800'}`}>
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${meshEnabled ? 'bg-emerald-500 text-white animate-pulse' : 'bg-slate-800 text-slate-500'}`}>
                                                <Shield size={20} />
                                            </div>
                                            <div>
                                                <h3 className="text-sm font-bold text-white flex items-center gap-2">
                                                    Guardian Mesh Protocol
                                                    {meshEnabled ? <span className="text-[10px] bg-emerald-500 text-white px-1.5 py-0.5 rounded-full animate-pulse uppercase tracking-widest">Active Protection</span> : <span className="text-[10px] bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded-full uppercase tracking-widest">Offline</span>}
                                                </h3>
                                                <p className="text-[11px] text-slate-400 mt-1">When active, your device contributes to the national community-based tracking mesh. You help others, others help you.</p>
                                            </div>
                                        </div>
                                        <button onClick={() => setMeshEnabled(!meshEnabled)} className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${meshEnabled ? 'bg-emerald-600 hover:bg-emerald-500 text-white' : 'bg-slate-800 hover:bg-slate-700 text-slate-400'}`}>
                                            {meshEnabled ? 'Deactivate Guardian' : 'Activate Guardian'}
                                        </button>
                                    </div>
                                </div>
                                                    ...
                                                    <div className="bg-slate-950/50 rounded-xl p-4 border border-slate-800/80 relative">
                                                        ...
                                                        {activeCert && activeCert.isActive ? (
                                                            ...
                                                            <div className="flex gap-3 items-center">
                                                                ...
                                                                <div className="overflow-hidden">
                                                                    <div className="text-xs text-red-400 font-semibold truncate leading-tight">REVOKED and LOCKED</div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                ...
                                <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <div>
                                            <h3 className="text-xl font-bold text-white">AI Scam Shield</h3>
                                            <p className="text-xs text-slate-400">Analyze messages for fraudulent patterns.</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <textarea
                                            ...
                                            className="w-full bg-slate-950 border border-slate-800 rounded-2xl p-4 text-sm text-slate-200 focus:outline-none focus:border-purple-500 min-h-[120px] transition-all"
                                        />
                                        <button
                                            onClick={runScamShield}
                                            disabled={isAnalyzingScam || !scamText}
                                            className="w-full bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-purple-500/20 flex items-center justify-center gap-2"
                                        >
                                            {isAnalyzingScam ? 'Shielding...' : 'Scan for Threats'}
                                            {!isAnalyzingScam && <Shield size={16} />}
                                        </button>

                                        {scamResult && (
                                            <div className={`p-4 rounded-2xl border animate-in fade-in zoom-in-95 ${scamResult.isScam ? 'bg-red-500/10 border-red-500/30' : 'bg-emerald-500/10 border-emerald-500/30'}`}>
                                                <div className="flex justify-between items-center mb-2">
                                                    <span className={`text-[10px] font-black uppercase tracking-widest ${scamResult.isScam ? 'text-red-500' : 'text-emerald-500'}`}>
                                                        {scamResult.isScam ? '🚨 SCAM DETECTED' : '✅ LIKELY SAFE'}
                                                    </span>
                                                    <span className="text-[10px] text-slate-500 font-bold">Confidence: {scamResult.confidence}%</span>
                                                </div>
                                                <p className="text-sm font-bold text-white mb-2">{scamResult.warning}</p>
                                                <p className="text-xs text-slate-400 leading-relaxed italic border-l-2 border-slate-700 pl-3">"{scamResult.action}"</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl relative overflow-hidden">
                                    ...
                                    <div className="flex items-center gap-3 mb-6">
                                        ...
                                        <div>
                                            <h3 className="text-xl font-bold text-white">Sentinel Legal AI</h3>
                                            <p className="text-xs text-slate-400">Cybercrime Act and Civil Law Specialist.</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex gap-2">
                                            <input
                                                ...
                                                onKeyDown={e => e.key === 'Enter' && getLegalHelp()}
                                                placeholder="Ask about penalties, reporting, or legal rights..."
                                                className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-blue-500 transition-all font-medium"
                                            />
                                            <button
                                                onClick={getLegalHelp}
                                                disabled={isThinkingLegal || !legalQuery}
                                                className="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold px-4 py-3 rounded-xl transition-all shadow-lg shadow-blue-500/20"
                                            >
                                                {isThinkingLegal ? '...' : <PlusCircle size={18} />}
                                            </button>
                                        </div>

                                        {legalAdvice && (
                                            <div className="p-5 bg-slate-950 border border-slate-800 rounded-2xl space-y-3 animate-in fade-in slide-in-from-right-4">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></div>
                                                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Legal Counsel Analysis</span>
                                                </div>
                                                <p className="text-sm text-slate-300 leading-relaxed font-medium whitespace-pre-wrap">{legalAdvice}</p>
                                                <div className="pt-3 border-t border-slate-800">
                                                    <p className="text-[10px] text-slate-600 italic">Disclaimer: AI guidance only. Consult a lawyer for binding advice.</p>
                                                </div>
                                            </div>
                                        )}
                                        {!legalAdvice && !isThinkingLegal && (
                                            <div className="p-8 text-center bg-slate-950/50 border border-dashed border-slate-800 rounded-2xl">
                                                <p className="text-xs text-slate-600 font-bold uppercase tracking-widest mb-2">Example Queries</p>
                                                <div className="flex flex-wrap justify-center gap-2">
                                                    <button onClick={() => setLegalQuery("What is the penalty for buying a stolen phone?")} className="text-[10px] bg-slate-800/50 text-slate-400 px-3 py-1.5 rounded-full hover:bg-slate-800 transition-colors">"Buying stolen phone penalty?"</button>
                                                    <button onClick={() => setLegalQuery("How do I report a suspected syndicate?")} className="text-[10px] bg-slate-800/50 text-slate-400 px-3 py-1.5 rounded-full hover:bg-slate-800 transition-colors">"Reporting a syndicate?"</button>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                        ...
                        {activeTab === 'escrow' &&
                            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="bg-indigo-600/10 border border-indigo-500/30 rounded-3xl p-8 shadow-2xl relative overflow-hidden mb-8">
                                    <div className="absolute top-0 right-0 p-8 opacity-10">
                                        <Shield size={120} className="text-indigo-500" />
                                    </div>
                                    <div className="max-w-2xl relative z-10">
                                        <h2 className="text-3xl font-black text-white mb-4">Safe-Trade™ Escrow System</h2>
                                        <p className="text-indigo-200 text-lg leading-relaxed mb-6">Trade devices with strangers safely. We hold the payment cryptographically until the physical handover is verified via the PTS Ledger.</p>
                                        <div className="flex items-center gap-4">
                                            <div className="flex -space-x-3">
                                                {[1, 2, 3, 4].map(i => <div key={i} className="w-10 h-10 rounded-full border-2 border-slate-900 bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-500 overflow-hidden"><img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${i + 10}`} alt="user" /></div>)}
                                            </div>
                                            <span className="text-sm font-bold text-indigo-300">Over 1,200 successful trades today.</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
                                        <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2"><Lock size={20} className="text-indigo-500" /> Start a Safe Trade</h3>
                                        <p className="text-sm text-slate-400 mb-8">To initiate, select 'Transfer Asset' on one of your active devices and toggle 'Enable Safe-Trade Escrow'.</p>
                                        <button onClick={() => setActiveTab('active')} className="w-full bg-slate-800 hover:bg-slate-700 text-white font-bold py-4 rounded-2xl transition-all border border-slate-700 flex items-center justify-center gap-3">
                                            View My Assets to Start
                                            <Smartphone size={18} />
                                        </button>
                                    </div>

                                    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl">
                                        <h3 className="text-xl font-bold text-white mb-6 flex items-center gap-2"><History size={20} className="text-indigo-500" /> Active Trade Locks</h3>
                                        {pendingTransfers.some(tx => tx.isEscrowEnabled) ? (
                                            <div className="space-y-4">
                                                {pendingTransfers.filter(tx => tx.isEscrowEnabled).map(tx => (
                                                    <div key={tx.id} className="p-4 bg-slate-950 rounded-2xl border border-indigo-500/20">
                                                        <div className="flex justify-between items-center mb-2">
                                                            <span className="text-[10px] bg-indigo-500 text-white px-2 py-0.5 rounded-full font-black uppercase tracking-widest">{tx.escrowStatus}</span>
                                                            <span className="text-sm font-black text-white">₦{tx.escrowAmount?.toLocaleString()}</span>
                                                        </div>
                                                        <p className="text-sm font-bold text-slate-300">{tx.device.brand} {tx.device.model}</p>
                                                        <p className="text-[10px] text-slate-500 mt-1 uppercase font-bold tracking-tighter">REF: {tx.id.substring(0, 8)}</p>
                                                    </div>
                                                ))}
                                            </div>
                                        ) : (
                                            <div className="p-10 text-center border-2 border-dashed border-slate-800 rounded-2xl">
                                                <p className="text-slate-600 font-bold uppercase tracking-widest text-xs">No Active Escrow Transactions</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    <button type="submit" disabled={isRegistering} className="w-full mt-6 bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-400 text-white font-bold py-4 rounded-xl transition-all shadow-lg hover:shadow-amber-500/20 disabled:opacity-50 flex items-center justify-center gap-3">
                                        {isRegistering ? (
                                            <span className="flex items-center gap-3">
                                                ...
                                                <span>{'Securing Device on Blockchain...'}</span>
                                            </span>
                                        ) : (
                                            <span>{'Verify and Generate Certificates'}</span>
                                        )}
                                    </button>
                                </form>
                            </div>
            ...
            <div style={{ position: 'absolute', top: '0', left: '-9999px', pointerEvents: 'none' }}>
                {certificateData && (
                    <div ref={certificateRef} style={{ width: '794px', height: '1123px' }} className="bg-white text-slate-900 relative p-14 font-serif flex flex-col justify-between overflow-hidden">
                        ...
                        <div className="relative z-10 h-full flex flex-col p-16">
                            ...
                            <div className="text-center mb-12">
                                <div className="inline-flex flex-col items-center mb-8">
                                    <div className="w-24 h-24 bg-slate-900 text-white rounded-2xl flex items-center justify-center shadow-2xl mb-6 relative">
                                ...
                                <div className="bg-slate-50/50 border border-slate-200 rounded-3xl p-10 mx-6 text-left shadow-md relative">
                                    ...
                                    <div className="grid grid-cols-2 gap-y-8 gap-x-12">
                                        ...
                                        <div className="col-span-2 pt-6 border-t border-slate-200">
                                            <p className="text-[10px] text-slate-400 uppercase tracking-widest font-black mb-2">IMEI Identity Number (International Standard)</p>
                                            <div className="flex items-center gap-4">
                            ...
                            <div className="mt-16 bg-slate-900 text-white rounded-[2.5rem] p-10 flex items-center justify-between shadow-2xl border-b-[8px] border-emerald-600">
                                ...
                                <div className="ml-4 w-36 h-36 bg-white rounded-2xl p-3 flex-shrink-0 flex items-center justify-center shadow-xl">
                                    <img
